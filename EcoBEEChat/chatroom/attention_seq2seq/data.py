...
def load_data():
    # 판다스를 통해서 데이터를 불러온다.
    data_df = pd.read_csv(DEFINES.data_path_attention_seq2seq, header=0)
...
def enc_processing(value, dictionary):
    ...
    sequences_length = []
    # 형태소 토크나이징 사용 유무
    if DEFINES.tokenize_as_morph_attention_seq2seq:
    ...
    for sequence in value:
        ...
        if len(sequence_index) > DEFINES.max_sequence_length_attention_seq2seq:
            sequence_index = sequence_index[:DEFINES.max_sequence_length_attention_seq2seq]
        # 하나의 문장에 길이를 넣어주고 있다.
        sequences_length.append(len(sequence_index))
        # max_sequence_length보다 문장 길이가
        # 작다면 빈 부분에 PAD(0)를 넣어준다.
        sequence_index += (DEFINES.max_sequence_length_attention_seq2seq -
...
def dec_target_processing(value, dictionary):
    ...
    for sequence in value:
        ...
        # 그리고 END 토큰을 넣어 준다
        if len(sequence_index) >= DEFINES.max_sequence_length_attention_seq2seq:
            sequence_index = sequence_index[:DEFINES.max_sequence_length_attention_seq2seq-1] + [
                dictionary[END]]
        else:
            sequence_index += [dictionary[END]]

        # 학습시 PAD 마스크를 위한 벡터를 구성한다.
        sequences_length.append([PAD_MASK if num > len(sequence_index) else NON_PAD_MASK for num in range(
            DEFINES.max_sequence_length_attention_seq2seq)])
        # max_sequence_length보다 문장 길이가
        # 작다면 빈 부분에 PAD(0)를 넣어준다.
        sequence_index += (DEFINES.max_sequence_length_attention_seq2seq -
                           len(sequence_index)) * [dictionary[PAD]]
...
def pred2string(value, dictionary):
    ...
    if DEFINES.serving_attention_seq2seq == True:
...
def load_vocabulary():
    ...
    # 그 파일의 존재 유무를 확인한다.
    if (not (os.path.exists(DEFINES.vocabulary_path_attention_seq2seq))):
        # 이미 생성된 사전 파일이 존재하지 않으므로
        # 데이터를 가지고 만들어야 한다.
        # 그래서 데이터가 존재 하면 사전을 만들기 위해서
        # 데이터 파일의 존재 유무를 확인한다.
        if (os.path.exists(DEFINES.data_path_attention_seq2seq)):
            # 데이터가 존재하니 판단스를 통해서
            # 데이터를 불러오자
            data_df = pd.read_csv(
                DEFINES.data_path_attention_seq2seq, encoding='utf-8')
            # 판다스의 데이터 프레임을 통해서
            # 질문과 답에 대한 열을 가져 온다.
            question, answer = list(data_df['question']), list(data_df['answer'])
            if DEFINES.tokenize_as_morph_attention_seq2seq:  # 형태소에 따른 토크나이져 처리
        ...
        # 사전 파일을 만들어 넣는다.
        with open(DEFINES.vocabulary_path_attention_seq2seq, 'w', encoding='utf-8') as vocabulary_file:
            for word in words:
                vocabulary_file.write(word + '\n')

    # 사전 파일이 존재하면 여기에서
    # 그 파일을 불러서 배열에 넣어 준다.
    with open(DEFINES.vocabulary_path_attention_seq2seq, 'r', encoding='utf-8') as vocabulary_file:
