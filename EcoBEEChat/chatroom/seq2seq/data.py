...
# 나누어 그 값을 리턴한다.
def load_data():
    data_df = pd.read_csv(DEFINES.data_path_seq2seq, header=0)
...
def enc_processing(value, dictionary):
    ...
    sequences_length = []
    # 형태소 토크나이징 사용 유무
    if DEFINES.tokenize_as_morph_seq2seq:
    ...
    for sequence in value:
        ...
        if len(sequence_index) > DEFINES.max_sequence_length_seq2seq:
            sequence_index = sequence_index[:
                                            DEFINES.max_sequence_length_seq2seq]

        sequences_length.append(len(sequence_index))
        # max_sequence_length보다 문장 길이가 작다면 빈 부분에 PAD(0)를 넣어준다.
        sequence_index += (DEFINES.max_sequence_length_seq2seq -
...
def dec_target_processing(value, dictionary):
    ...
    for sequence in value:
        ...
        # 그리고 END 토큰을 넣어 준다
        if len(sequence_index) >= DEFINES.max_sequence_length_seq2seq:
            sequence_index = sequence_index[:DEFINES.max_sequence_length_seq2seq-1] + [
                dictionary[END]]
        else:
...
def load_vocabulary():
    vocabulary_list = []
    # 사전 파일의 존재 유무를 확인한다.
    if (not (os.path.exists(DEFINES.vocabulary_path_seq2seq))):
        if (os.path.exists(DEFINES.data_path_seq2seq)):
            data_df = pd.read_csv(DEFINES.data_path_seq2seq, encoding='utf-8')
            question, answer = list(data_df['question']), list(data_df['answer'])
            if DEFINES.tokenize_as_morph_seq2seq:
        ...
        with open(DEFINES.vocabulary_path_seq2seq, 'w', encoding='utf-8') as vocabulary_file:
            for word in words:
                vocabulary_file.write(word + '\n')

    # 사전 파일이 존재하면 여기에서 그 파일을 불러서 배열에 넣어 준다.
    with open(DEFINES.vocabulary_path_seq2seq, 'r', encoding='utf-8') as vocabulary_file:
